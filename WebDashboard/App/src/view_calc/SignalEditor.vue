...
const isStruct = computed(() => model.value.Type === 'Struct')

const showStructColumns = computed(
  () => isStruct.value && model.value.Dimension === 1 && !showRawJson.value,
)
...
watch(tab, (newTab) => {
  if (newTab === 'history' && historyRows.value.length === 0) {
    latestDesc()
  }
})
