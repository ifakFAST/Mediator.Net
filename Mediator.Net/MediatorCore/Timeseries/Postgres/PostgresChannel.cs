...
namespace Ifak.Fast.Mediator.Timeseries.Postgres
{
    public class PostgresChannel : Channel
    {
        ...
        public PostgresChannel(DbConnection connection, ChannelInfo info, string tableName) {
            ...
            stmtCountNonBad     = new PreparedStatement(connection, $"SELECT COUNT(*) FROM {table} WHERE time BETWEEN @1 AND @2 AND quality <> 0", 2, time, time);
            stmtCountGood       = new PreparedStatement(connection, $"SELECT COUNT(*) FROM {table} WHERE time BETWEEN @1 AND @2 AND quality = 1", 2, time, time);
        }
