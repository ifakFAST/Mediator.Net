...
public static class VTQ_Serializer
{
    ...
    public static void Serialize(Stream stream, List<VTQ> varVals, byte binaryVersion) {
        using var writer = new BinaryWriter(stream, Encoding.UTF8, leaveOpen: true);
        Serialize(writer, varVals, binaryVersion);
    }

    public static void Serialize(BinaryWriter writer, List<VTQ> vtqs, byte binaryVersion) {

        int N = vtqs.Count;
        if (N > Common.MaxListLen) throw new System.Exception($"VTQ_Serializer: May not serialize more than {Common.MaxListLen} items");
    ...
    public static void SerializeVTTQ(Stream stream, List<VTTQ> varVals, byte binaryVersion) {
        using var writer = new BinaryWriter(stream, Encoding.UTF8, leaveOpen: true);
        SerializeVTTQ(writer, varVals, binaryVersion);
    }

    public static void SerializeVTTQ(BinaryWriter writer, List<VTTQ> vtqs, byte binaryVersion) {
    ...
    public static List<VTQ> Deserialize(BinaryReader reader) {
        ...
        if (binaryVersion == 0) throw new IOException("Failed to deserialize VTQ[]: Version byte is zero");
        if (binaryVersion > Common.CurrentBinaryVersion) throw new IOException("Failed to deserialize VTQ[]: Wrong version byte");
        if (reader.ReadByte() != Code) throw new IOException("Failed to deserialize VTQ[]: Wrong start byte");

        int N = reader.ReadInt32();
        if (N > Common.MaxListLen) throw new System.Exception($"VTQ_Serializer: May not deserialize more than {Common.MaxListLen} items");
    ...
    public static List<VTTQ> DeserializeAsVTTQ(Stream stream) {
        using var reader = new BinaryReader(stream, Encoding.UTF8, leaveOpen: true);
        return DeserializeAsVTTQ(reader);
    }

    public static List<VTTQ> DeserializeAsVTTQ(BinaryReader reader) {
        ...
        for (int k = 0; k < N; ++k) {
            ...
            Timestamp t = Timestamp.FromJavaTicks(time);
            res.Add(VTTQ.Make(DataValue.FromJSON(val), t, t, q));
