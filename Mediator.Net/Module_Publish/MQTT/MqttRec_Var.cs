...
public partial class MqttPublisher
{
    public static async Task MakeVarRecTask(MqttConfig config, ModuleInitInfo info, string certDir, Func<bool> shutdown) {
        ...
        Connection clientFAST = await Util.EnsureConnectOrThrow(info, null, onConfigChanged, varRec.ModuleID);
        ...
        while (!shutdown()) {
            ...
            clientFAST = await Util.EnsureConnectOrThrow(info, clientFAST, onConfigChanged, varRec.ModuleID);
