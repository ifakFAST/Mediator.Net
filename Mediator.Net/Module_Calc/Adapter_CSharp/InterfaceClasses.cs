...
public class Alarm : AbstractState, EventProvider
{
    ...
    private readonly bool prefixMessageWithName = true;
...
public class Api
{
    ...
    public void HistorianModify(VariableRef variable, ModifyMode mode, VTQ[] data) {
        ...
        SingleThreadedAsync.Run(async () => {
            try {
                Connection con = await connectionGetter();
                await con.HistorianModify(variable, mode, data);
            }
            catch (Exception exp) {
                Exception e = exp.GetBaseException() ?? exp;
                errMsg = e.Message;
    ...
    private ObjectRef CreateObjectIfNotExists(string parentFolderID, ObjectWithID newObjInfo) {

        var newObjID = ObjectRef.Make(moduleID, newObjInfo.ID);
        var parentObjID = ObjectRef.Make(moduleID, parentFolderID);
        ...
        SingleThreadedAsync.Run(async () => {
            try {
                ...
                var objects = await con.GetObjectsByID([newObjID], ignoreMissing: true);
                bool signalExists = objects.Count == 1;

                if (!signalExists) {
                    DataValue newObj = DataValue.FromObject(newObjInfo);
                    AddArrayElement elem = AddArrayElement.Make(parentObjID, nameof(Folder.Signals), newObj);
                    await con.UpdateConfig(elem);
                }
            }
            catch (Exception exp) {
                Exception e = exp.GetBaseException() ?? exp;
